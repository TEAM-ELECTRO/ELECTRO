...
@ElectroBot.on(admin_cmd(pattern=r"unpack", outgoing=True))
@ElectroBot.on(sudo_cmd(pattern=r"unpack"))
async def _(event):
...
@ElectroBot.on(admin_cmd(pattern="repack ?(.*)", outgoing=True))
@ElectroBot.on(sudo_cmd(pattern="repack ?(.*)", allow_sudo=True))
async def _(event):
