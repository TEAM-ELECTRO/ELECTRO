...
from ElectroBot.utils import admin_cmd, sudo_cmd, edit_or_reply, progress
...
async def nope(aura):
    Electro = aura.pattern_match.group(1)
    if not Electro:
        if aura.is_reply:
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(Electro))}")
    ...
    async with borg.conversation("@DeezLoadBot") as conv:
        try:
            ...
            await borg.send_file(
                event.chat_id,
                songh,
                caption="🔆**Here's the requested song!**🔆\n`Check out` [ElectroBot](https://t.me/ElectroSupport)",
            )
            await event.delete()
        except YouBlockedUserError:
