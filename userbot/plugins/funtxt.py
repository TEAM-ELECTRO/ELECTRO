...
async def hmm(Electro):
    if Electro.fwd_from:
        return
    reactcat = nekos.textcat()
    await edit_or_reply(Electro, reactcat)
...
async def hmm(Electro):
    if Electro.fwd_from:
        return
    whyElectro = nekos.why()
    await edit_or_reply(Electro, whyElectro)
...
async def hmm(Electro):
    if Electro.fwd_from:
        return
    factElectro = nekos.fact()
    await edit_or_reply(Electro, factElectro)
