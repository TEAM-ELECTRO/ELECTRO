...
from ElectroBot.utils import admin_cmd, edit_or_reply, progress, sudo_cmd
...
async def _(Electro):
    if Electro.fwd_from:
        return
    reply_to_id = Electro.message.id
    if Electro.reply_to_msg_id:
        reply_to_id = Electro.reply_to_msg_id
    event = await edit_or_reply(Electro, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await Electro.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            caat = await Electro.client.send_file(
                event.chat_id,
...
async def _(event):
    if event.fwd_from:
        return
    Electroreply = await event.get_reply_message()
    if not Electroreply or not Electroreply.media or not Electroreply.media.document:
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    if Electroreply.media.document.mime_type != "application/x-tgsticker":
        return await edit_or_reply(event, "`Stupid!, This is not animated sticker.`")
    reply_to_id = event.message
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    chat = "@tgstogifbot"
    Electroevent = await edit_or_reply(event, "`Converting to gif ...`")
    async with event.client.conversation(chat) as conv:
        try:
            await silently_send_message(conv, "/start")
            await event.client.send_file(chat, Electroreply.media)
            response = await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            if response.text.startswith("Send me an animated sticker!"):
                return await Electroevent.edit("`This file is not supported`")
            Electroresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            Electrofile = Path(await event.client.download_media(Electroresponse, "./temp/"))
            Electrogif = Path(await unzip(Electrofile))
            Electro = await event.client.send_file(
                event.chat_id,
                Electrogif,
            ...
            await event.client(
                functions.messages.SaveGifRequest(
                    id=types.InputDocument(
                        id=Electro.media.document.id,
                        access_hash=Electro.media.document.access_hash,
                        file_reference=Electro.media.document.file_reference,
                    ),
                    unsave=True,
                )
            )
            await Electroevent.delete()
            for files in (Electrogif, Electrofile):
                if files and os.path.exists(files):
                    os.remove(files)
        except YouBlockedUserError:
            await Electroevent.edit("Unblock @tgstogifbot")
            return
