...
async def download(event):
    ...
    if Var.GIT_REPO_NAME is None:
        await edit_or_reply(event, "`Please ADD Proper Github Repo Name of ElectroBot`")
        return
    ElectroBot = await edit_or_reply(event, "Processing ...")
    ...
    try:
        ...
        await ElectroBot.edit(str(e))
    else:
        end = datetime.now()
        ms = (end - start).seconds
        await event.delete()
        await ElectroBot.edit(
            "Downloaded to `{}` in {} seconds.".format(downloaded_file_name, ms)
        )
        await ElectroBot.edit("Committing to Github....")
        await git_commit(downloaded_file_name, ElectroBot)


async def git_commit(file_name, ElectroBot):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await ElectroBot.edit("`File Already Exists`")
            create_file = False
    ...
    if create_file == True:
        ...
        try:
            ...
            await ElectroBot.edit("Cannot Upload Plugin")
    else:
        return await ElectroBot.edit("`Committed Suicide`")
