...
async def _(event):
    ...
    Electroevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_sElectro(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    Electrouser = await event.client.get_me()
    if Electrouser.username:
        curruser = Electrouser.username
    else:
        curruser = "ElectroBot"
    uid = os.geteuid()
    if uid == 0:
...
async def _(event):
    ...
    Electroevent = await edit_or_reply(event, "`Running ...`")
...
CmdHelp("evaluators").add_command(
  ...
  'exec', '<command>', 'Execute a Terminal command on ElectroBot server and shows details'
).add_command(
