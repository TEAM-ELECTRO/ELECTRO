...
ElectroBot = Config.CUSTOM_STICKER_PACK_NAME
...
async def kang(args):
    ...
    if photo:
        ...
        packname = f"AB_{user.username}_ElectroBot_{pack}"
        packnick = (
            f"{ElectroBot} Vol.{pack}"
            if ElectroBot
            else f"@{user.username}'s ElectroBot Vol.{pack}"
        )
        cmd = "/newpack"
        file = io.BytesIO()
