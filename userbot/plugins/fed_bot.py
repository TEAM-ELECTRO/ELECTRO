...
from ElectroBot import CmdHelp, bot as ElectroBot
from ElectroBot.utils import admin_cmd, sudo_cmd, edit_or_reply as eor
from ElectroBot.Config import Config
from ElectroBot.plugins.sql_helper.fban_sql import (
    add_channel,
    get_all_channels,
    in_channels,
    rm_channel,
...
bot = "@MissRose_bot"
Electro_logo = "./Electro/ElectroBot_logo.jpg"
# Keep all credits pls
# madewith great effort by @HeisenbergTheDanger
# modified by @Kraken_The_BadAss for fbans


@ElectroBot.on(admin_cmd(pattern="fban ?(.*)"))
@ElectroBot.on(sudo_cmd(pattern="fban ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@ElectroBot.on(admin_cmd(pattern="unfban ?(.*)"))
@ElectroBot.on(sudo_cmd(pattern="unfban ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mssg = await eor(event, "`...`")
...
@ElectroBot.on(admin_cmd(pattern=r"fadd ?(.*)"))
@ElectroBot.on(sudo_cmd(pattern=r"fadd ?(.*)", allow_sudo=True))
...
@ElectroBot.on(admin_cmd(pattern=r"fremove ?(.*)"))
@ElectroBot.on(sudo_cmd(pattern=r"fremove ?(.*)", allow_sudo=True))
...
@ElectroBot.on(admin_cmd(pattern="fgroups"))
@ElectroBot.on(sudo_cmd(pattern="fgroups", allow_sudo=True))
...
@ElectroBot.on(admin_cmd(pattern="fsearch ?(.*)"))
@ElectroBot.on(sudo_cmd(pattern="fsearch ?(.*)", allow_sudo=True))
...
@ElectroBot.on(admin_cmd(pattern="newfed ?(.*)", outgoing=True))
@ElectroBot.on(sudo_cmd(pattern="newfed ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Electro_input = event.pattern_match.group(1)
    chat = "@MissRose_Bot"
    ...
    async with borg.conversation(chat) as conv:
        try:
            ...
            await event.client.send_message(chat, f"/newfed {Electro_input}")
...
@ElectroBot.on(admin_cmd(pattern="renamefed ?(.*)"))
@ElectroBot.on(sudo_cmd(pattern="renamefed ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return 
    Electro_input = event.pattern_match.group(1)
    ...
    async with event.client.conversation(chat) as conv:
          try:     
              ...
              await event.client.send_message(chat, f"/renamefed {Electro_input}")
              response = await response 
          except YouBlockedUserError: 
...
@ElectroBot.on(admin_cmd(pattern="fstat ?(.*)"))
@ElectroBot.on(sudo_cmd(pattern="fstat ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Electro = await event.edit("`Collecting fstat....`")
    thumb = Electro_logo
    if event.reply_to_msg_id:
        previous_message = await event.get_reply_message()
        ...
        async with borg.conversation(bot) as conv:
            try:
                ...
                if "Looks like" in massive.text:
                    ...
                    massive = await conv.get_response()
                    await ElectroBot.send_file(
                        event.chat_id,
                        massive,
                        thumb=thumb,
                        caption=f"List of feds {user} has been banned in.\n\n**⚡ [Collected using ElectroBot](t.me/ElectroUserbot) ⚡**",
                    )
                else:
                    await borg.send_message(event.chat_id, massive.text)
                await event.delete()
            except YouBlockedUserError:
                await Electro.edit("`Please Unblock` @MissRose_Bot")


@ElectroBot.on(admin_cmd(pattern="fedinfo ?(.*)"))
@ElectroBot.on(sudo_cmd(pattern="fedinfo ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    Electro = await eor(event, "`Fetching fed info.... please wait`")
    lavde = event.pattern_match.group(1)
    async with borg.conversation(bot) as conv:
        try:
            await conv.send_message("/start")
            ...
            massive = await conv.get_response()
            await Electro.edit(massive.text + "\n\n**LEGENDARY_AF_ElectroBot**")
        except YouBlockedUserError:
            await Electro.edit("`Please Unblock` @MissRose_Bot")
