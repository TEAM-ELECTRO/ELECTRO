...
if Config.TAG_LOGGER:
    ...
    async def all_messages_catcher(event):
        ...
        Electro = await event.client.get_entity(event.sender_id)
        if Electro.bot or Electro.verified or Electro.support:
            return

        Electrom = f"[{get_display_name(Electro)}](tg://user?id={Electro.id})"
