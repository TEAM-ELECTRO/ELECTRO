...
async def bigspam(Electro):
    if not Electro.text[0].isalpha() and Electro.text[0] not in ("/", "#", "@", "!"):
        Electro_msg = Electro.text
        ElectroBot_count = int(Electro_msg[9:13])
        Electro_spam = str(Electro.text[13:])
        for i in range(1, ElectroBot_count):
            await Electro.respond(Electro_spam)
        await Electro.delete()
        if LOGGER:
            await Electro.client.send_message(
...
CmdHelp("spam").add_command(
  ...
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. This what ElectroBot iz known for. The Best BigSpam Ever", ".bigspam 5000 Hello"
).add()
