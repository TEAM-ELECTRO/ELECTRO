...
Electro_NAME = str(ALIVE_NAME) if ALIVE_NAME else "Electro User"
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        if input_str == "m":
            ...
            try:
                ...
                await edit_or_reply(event, 
                   "✓ **YOUR FILE :-** https://telegra.ph{} \n✓ **Time Taken :-** `{}` secs \n✓ **By :- [{}](tg://user?id={})**".format(
                        media_urls[0], (ms + ms_two), Electro_NAME, Electro
                    ),
                    link_preview=True,
                )
        elif input_str == "t":
            ...
            ms = (end - start).seconds
            IisGElectrov = f"https://telegra.ph/{response['path']}"
            await edit_or_reply(event, 
                  f"✓ **Pasted to** [telegraph]({IisGElectrov}) \n✓ **Time Taken :-** `{ms}` secs\n✓** By :- **[{Electro_NAME}](tg://user?id={Electro})", link_preview=True)
    else:
        await edit_or_reply(event, 
