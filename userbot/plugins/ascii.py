...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    Electro = await edit_or_reply(event, "Wait making ASCII...🤓🔥🔥")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await Electro.edit("`Please unblock @asciiart_bot and try again`")
            return
...
async def _(event):
    ...
    if reply_message.sender.bot:
        await edit_or_reply(event, "Reply to actual users message.😒🤐")
        return
    Electro = await edit_or_reply(event, "`Processing`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await Electro.edit("Please unblock @Lines50Bot and try again")
            return
        await Electro.delete()
        await event.client.send_file(
            event.chat_id,
