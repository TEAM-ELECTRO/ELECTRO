...
async def lg1(Electro):
    event = await edit_or_reply(Electro, "`Processing.....`")
    fnt = await get_font_file(Electro.client, "@VegaFonts")
    if Electro.reply_to_msg_id:
        rply = await Electro.get_reply_message()
    ...
    text = Electro.pattern_match.group(1)
    if len(text) <= 8:
        font_size_ = 150
        strik = 10
    elif len(text) >= 9:
    ...
    file_name = "ElectroBot.png"
    img.save(file_name, "png")
    await bot.send_file(
        Electro.chat_id,
        file_name,
        caption=f"**Made By :** [{DEFAULTUSER}](tg://user?id={Electro})",
    )
    await event.delete()
    try:
        os.remove(file_name)
