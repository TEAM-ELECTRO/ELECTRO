...
async def get_users(event):
    ...
    if not sender.id == me.id:
        ...
        Electro = await edit_or_reply(event, "`processing...`")
    Electro = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await Electro.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await Electro.edit("**TerminalStatus**\n\n`Collecting Users.......`")
    async for user in event.client.iter_participants(Electro.full_chat.id):
        try:
            if error.startswith("Too"):
                return await Electro.edit(
