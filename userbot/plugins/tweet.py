...
async def nope(Electro):
    ...
    if not Electro:
        if Electro.is_reply:
            ...
            await edit_or_reply(Electro, "I need some text to make a tweet🚶")
            return
    tweeter = await bot.inline_query("TwitterStatusBot", f"{(deEmojify(Electro))}")
